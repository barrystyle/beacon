...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_17_FAIR_PAY_CHECKS) r = SPORK_17_FAIR_PAY_CHECKS_DEFAULT;
...
bool CSporkManager::CheckSignature(CSporkMessage& spork, bool fRequireNew)
{
    ...
    bool fValidSpork = obfuScationSigner.VerifyMessage(pubkeynew, spork.vchSig, strMessage, errorMessage);
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_17_FAIR_PAY_CHECKS") return SPORK_17_FAIR_PAY_CHECKS;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_17_FAIR_PAY_CHECKS) return "SPORK_17_FAIR_PAY_CHECKS";
